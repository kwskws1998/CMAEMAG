...
def compute_fixation_trial_level_features(
    ...
    trial_ia_counts = trial['TRIAL_IA_COUNT'].drop_duplicates().dropna().values
    if len(trial_ia_counts) > 0:
        BEYELSTM['total_num_words'] = trial_ia_counts[0]
    else:
        BEYELSTM['total_num_words'] = (
            trial['CURRENT_FIX_INTEREST_AREA_INDEX'].dropna().nunique()
        )
