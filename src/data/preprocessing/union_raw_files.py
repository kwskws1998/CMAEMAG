...
PYMOVEMENTS_DATASET_ALIASES = {
    DataSets.HALLUCINATION: 'IITB_HGC',
}


def get_pymovements_dataset_name(dataset_name: str) -> str:
    """Return the dataset identifier used by the installed pymovements version."""
    return PYMOVEMENTS_DATASET_ALIASES.get(dataset_name, dataset_name)
...
def combine_dataset(dataset_name: str) -> None:
    ...
    if dataset_name == 'MECOL2':
        ...
        for part in ('W1', 'W2'):
            ...
            dataset_def = pm.DatasetLibrary.get(
                get_pymovements_dataset_name(f'{dataset_name}{part}')
            )
        ...
        dataset_def = pm.DatasetLibrary.get(get_pymovements_dataset_name(dataset_name))
