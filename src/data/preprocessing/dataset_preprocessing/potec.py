...
class PoTeCProcessor(DatasetProcessor):
    ...
    def add_ia_report_features_to_fixation_data(
        ...
        if word_aoi_df.empty:
            if Fields.IA_DATA_IA_ID_COL_NAME not in fix_df.columns:
                raise ValueError(
                    'Cannot map PoTeC fixations to interest areas. Missing '
                    f'{Fields.IA_DATA_IA_ID_COL_NAME} and word AOI files.'
                )
            fix_df['CURRENT_FIX_INTEREST_AREA_INDEX'] = pd.to_numeric(
                fix_df[Fields.IA_DATA_IA_ID_COL_NAME],
                errors='coerce',
            )
            fix_df['word'] = fix_df.get('word', '.')
            fix_df['start_of_line'] = False
            fix_df['end_of_line'] = False
        else:
        ...
        if 'TRIAL_IA_COUNT' in enriched_fix_df.columns:
            enriched_fix_df['TRIAL_IA_COUNT'] = enriched_fix_df[
                'TRIAL_IA_COUNT'
            ].fillna(enriched_fix_df['num_of_words_in_trial'])
        else:
            enriched_fix_df['TRIAL_IA_COUNT'] = enriched_fix_df[
                'num_of_words_in_trial'
            ]
    ...
    def _read_word_aoi_df(self) -> pd.DataFrame:
        ...
        if not os.path.isdir(stim_path):
            logger.warning(
                f'{stim_path} is missing. Falling back to PoTeC fixation AOI ids.'
            )
            return pd.DataFrame()
        ...
        if not dfs:
            logger.warning(
                f'No word_aoi_*.tsv files found in {stim_path}. '
                'Falling back to PoTeC fixation AOI ids.'
            )
            return pd.DataFrame()
    ...
    def _load_and_merge_labels(self, df: pd.DataFrame) -> pd.DataFrame:
        # participant data
        participant_data_path = 'data/PoTeC/labels/participant_data.tsv'
        if os.path.exists(participant_data_path):
            label_df = pd.read_csv(participant_data_path, delimiter='\t').rename(
                columns={'reader_id': Fields.SUBJECT_ID}
            )
            ...
            logger.warning(
                f'{participant_data_path} is missing. '
                'Using participant columns already present in PoTeC precomputed files.'
            )

        # response accuracy data
        response_accuracy_path = 'data/PoTeC/labels/participant_response_accuracy.tsv'
        if os.path.exists(response_accuracy_path):
            text_spec_df = pd.read_csv(response_accuracy_path, delimiter='\t').rename(
                columns={
                    'reader_id': Fields.SUBJECT_ID,
                    'text_id': Fields.UNIQUE_PARAGRAPH_ID,
                }
            ...
            drop_cols = (set(df.columns) & set(text_spec_df.columns)) - set(
                merge_keys
            )
            df = df.merge(
            ...
            logger.warning(
                f'{response_accuracy_path} is missing. '
                'Using accuracy columns already present in PoTeC precomputed files.'
            )

        bq_cols = ['acc_bq_1', 'acc_bq_2', 'acc_bq_3']
        missing_bq_cols = [col for col in bq_cols if col not in df.columns]
        if missing_bq_cols:
            if 'mean_acc_bq' not in df.columns:
                raise ValueError(
                    'Cannot compute PoTeC DE label. Missing columns: '
                    f'{missing_bq_cols}'
                )
            df['mean_acc_bq'] = pd.to_numeric(df['mean_acc_bq'], errors='coerce')
        else:
            df[bq_cols] = df[bq_cols].apply(pd.to_numeric, errors='coerce')
            df['mean_acc_bq'] = df[bq_cols].mean(axis=1)
    ...
    def _load_question_map(self) -> dict[str, str]:
        """Load PoTeC comprehension questions from bundled fold metadata."""
        trial_ids_path = 'data/PoTeC/folds_metadata/trial_ids'
        if not os.path.isdir(trial_ids_path):
            return {}

        dfs = []
        for file_name in os.listdir(trial_ids_path):
            if not file_name.endswith('_trial_ids_by_regime.csv'):
                continue
            file_path = os.path.join(trial_ids_path, file_name)
            fold_df = pd.read_csv(
                file_path,
                usecols=['unique_trial_id', 'question'],
            )
            dfs.append(fold_df)
        ...
        question_df = pd.concat(dfs, ignore_index=True).dropna(
            subset=['unique_trial_id', 'question']
        )
        question_df = question_df.drop_duplicates(
            subset=['unique_trial_id'],
            keep='first',
        )
        return dict(
            zip(
                question_df['unique_trial_id'].astype(str),
                question_df['question'].astype(str),
            )
        )
    ...
    def _expand_for_questions(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for q in tqdm(question_indices, desc='Expanding for questions'):
            df_temp = df.copy()
            raw_rc = pd.to_numeric(df_temp[f'acc_{q}'], errors='coerce')
            df_temp['RC'] = (raw_rc > 0.9).astype(float)
            df_temp['question_index'] = q
            df_temp['question'] = (
                df_temp[q]
                if q in df_temp.columns
                else f'PoTeC comprehension question {q}'
            )
            dfs.append(df_temp)
        ...
        if question_map:
            mapped_questions = df['unique_trial_id'].astype(str).map(question_map)
            df['question'] = mapped_questions.fillna(df['question'])
        else:
            logger.warning(
                'PoTeC trial fold metadata is missing. '
                'Using question text from stimuli or fallback placeholders.'
            )
    ...
    def dataset_specific_processing(
        ...
        for data_type in [DataType.IA, DataType.FIXATIONS]:
            ...
            # merge stimuli data
            stimuli_path = 'data/PoTeC/stimuli/stimuli.tsv'
            if os.path.exists(stimuli_path):
                stim_df = pd.read_csv(stimuli_path, delimiter='\t').rename(
                    columns={
                        'reader_id': Fields.SUBJECT_ID,
                        'text_id': Fields.UNIQUE_PARAGRAPH_ID,
                    }
                ...
                logger.warning(
                    f'{stimuli_path} is missing. '
                    'Using PoTeC fold metadata for comprehension question text.'
                )
